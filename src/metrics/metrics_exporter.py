...
from metrics.metrics_calculator import calculate_base_metrics

_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
_OUT  = os.path.join(_ROOT, "out")


def export_global_metrics_json(nodes: list[Any] | set[Any], edges: list[Any] | set[Any], output_path: str = "") -> None:
    if not output_path:
        output_path = os.path.join(_OUT, "global.json")
...
def export_regions_metrics_json(regions_data: dict[str, tuple[list[Any] | set[Any], list[Any] | set[Any]]], output_path: str = "") -> None:
    if not output_path:
        output_path = os.path.join(_OUT, "regioes.json")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    result = {}
...
def export_degrees_csv(degrees: dict[str, int], output_path: str = "") -> None:
    if not output_path:
        output_path = os.path.join(_OUT, "graus.csv")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
def export_ego_metrics(ego_data: dict[str, tuple[int, list[Any] | set[Any], list[Any] | set[Any]]], csv_path: str = "", json_path: str = "") -> None:
    if not csv_path:
        csv_path  = os.path.join(_OUT, "ego_aeroportos.csv")
    if not json_path:
        json_path = os.path.join(_OUT, "ego_aeroportos.json")
