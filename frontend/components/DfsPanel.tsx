...
import { runDfs, getDfsTreeEdges, dfsLevelColor } from '../lib/graph/dfs';
...
export function DfsPanel({
  ...
  graph: GraphData;
  onResult: (result: DfsResult | null) => void;
  onClose: () => void;
}) {
  ...
  function calculate() {
    ...
    const r = runDfs(graph, originKey);
    setResult(r);
    onResult(r);
  }
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden border-l border-zinc-200 bg-white">
      ...
      <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
        <span className="text-sm font-bold text-zinc-800">DFS</span>
            ...
            <div key={level}>
              <p className="sticky top-[33px] border-b border-zinc-100 bg-zinc-50 px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-zinc-600 flex items-center gap-2">
                <span
                  ...
                  style={{ backgroundColor: dfsLevelColor(level, result.maxLevel) }}
                />
