...
export function ChartsSheet({
  ...
  const [localBfs, setLocalBfs] = useState<BfsResult | null>(bfsResult);
  ...
  const outDeg = useMemo(() => computeOutDegrees(graph), [graph]);
  const inDeg = useMemo(() => computeInDegrees(graph), [graph]);
  const degrees = degreeDir === "out" ? outDeg : inDeg;

  const distData = useMemo(() => degreeDistribution(degrees), [degrees]);
  const rankData = useMemo(() => topAirports(outDeg, graph, rankLimit), [outDeg, graph, rankLimit]);
  const regionData = useMemo(() => regionalMetrics(graph), [graph]);
  const flowData = useMemo(() => regionalFlowMatrix(graph), [graph]);
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        dragElastic={0.08}
        onDragEnd={(_, info) => {
          if (info.velocity.y > 120 || info.offset.y > 180) onClose();
        }}
      >
        {/* Handle */}
        ...
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between border-b border-zinc-100 px-6 pb-3">
          <span className="text-base font-bold text-zinc-900">Análises do Grafo</span>
          <button
            onClick={onClose}
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={bfsData} margin={{ top: 10, right: 16, bottom: 8, left: 8 }}>
                      ...
                      <Bar
                        ...
                        radius={[3, 3, 0, 0]}
                        cursor="pointer"
                        onClick={(d) => setBfsSelectedLevel(bfsSelectedLevel === d.level ? null : d.level)}
                      >
                        {bfsData.map((d) => (
                          <Cell
                            key={d.level}
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={dfsData} margin={{ top: 10, right: 16, bottom: 8, left: 8 }}>
                      ...
                      <Bar
                        ...
                        radius={[3, 3, 0, 0]}
                        cursor="pointer"
                        onClick={(d) => setDfsSelectedLevel(dfsSelectedLevel === d.level ? null : d.level)}
                      >
                        {dfsData.map((d) => (
                          <Cell
                            key={d.level}
...
function LevelAirportList({
  ...
  level: number;
  airports: { key: string; city: string }[];
  color: string;
  onClose: () => void;
}) {
  return (
